...
if ($id != ""):
  ...
  if ($result->EOF):
    ...
    CONN::get()->Execute("UPDATE CD_PERSON SET is_active = 'S', pass = NULL, cd_valid = null, tent = 0 WHERE id = ?", array($cd_person));
  endif;
endif;
?>
<!DOCTYPE html>
...
<head>
  <meta charset="utf-8" />
  <title>Ministérios IASD - Esqueci a senha</title>
    ...
    <div class="loginbox bg-white">
      ...
      <form lass="form-signin" method="post" id="register-form"
          ...
          <div class="col-lg-12 loginbox-textbox">
            <span class="input-icon">
              ...
              <input class="form-control" name="email" id="email" type="email" placeholder="Email"
                ...
                <?= (empty($hint) ? "" : "value=\"$hint\" disabled=\"true\"") ?> />
            </span>
          </div>
        </div>
        <?php if (!empty($hint)): ?>
        ...
        <div class="loginbox-submit">
          <button type="submit" class="btn btn-danger btn-block"><span class="glyphicon glyphicon-off"></span>&nbsp;Enviar solicitação</button>
        </div>
