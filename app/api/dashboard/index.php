...
function painel()
{
  ...
  $rs = CONN::get()->Execute("SELECT COUNT(*) AS qt FROM CD_PERSON");
  $nrUsr = 0;
  if (!$rs->EOF):
    $nrUsr = $rs->fields["qt"];
    $arr[] = array(
  ...
  $rs = CONN::get()->Execute("SELECT * FROM HS_RESULTS WHERE tp = ?", array('D'));
  if (!$rs->EOF):
    $arr[] = array(
      "leftBkTheme" => "databox-left bg-palegreen",
      "pc" => min(ceil(($rs->RecordCount() / $nrUsr) * 100), 100),
  ...
  $rs = CONN::get()->Execute("SELECT * FROM HS_RESULTS WHERE tp = ?", array('M'));
  if (!$rs->EOF):
    $arr[] = array(
      "leftBkTheme" => "databox-left bg-palegreen",
      "pc" => min(ceil(($rs->RecordCount() / $nrUsr) * 100), 100),
